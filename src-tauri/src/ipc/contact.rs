...
use tauri::State;
use sea_orm::DbConn;
use crate::database::handler::{UserHandler, ContactHandler};
use crate::types::{UserInfo, Contact};
...
pub async fn get_contact_list_handler(
    ...
    let contacts = ContactHandler::list_by_owner(db, owner_uid)
        .await
...
pub async fn get_online_users_handler(
    ...
    let online_users = UserHandler::find_by_status(db, 1)
        .await
        .map_err(|e| e.to_string())?;

    let busy_users = UserHandler::find_by_status(db, 2)
        .await
        .map_err(|e| e.to_string())?;
