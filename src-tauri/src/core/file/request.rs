...
use crate::network::feiq::{
    constants::*,
    model::{FileAttachment, ProtocolPacket},
};

/// 处理接收到的文件附件请求
///
/// 当收到 IPMSG_SENDMSG | IPMSG_FILEATTACHOPT 消息时调用
pub fn handle_file_attach_request(packet: &ProtocolPacket) -> AppResult<Vec<FileAttachment>> {
    // 检查是否带文件附件
...
///
/// 用于发送文件给其他用户
pub fn create_file_attach_request(files: &[FileAttachment], receiver_ip: &str, receiver_port: u16) -> ProtocolPacket {
...
///
/// 用于接收方请求文件数据
pub fn create_file_data_request(packet_no: &str, file_id: u64, offset: u64) -> ProtocolPacket {
...
///
/// 用于通知发送方释放文件资源
pub fn create_file_release(packet_no: &str) -> ProtocolPacket {
